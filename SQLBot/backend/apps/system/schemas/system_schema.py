...
class BaseUserDTO(BaseUser, BaseCreatorDTO):
    language: str = Field(pattern=r"^(zh-CN|en|ko-KR|tr-TR)$", default="zh-CN", description="用户语言")
    password: str
    status: int = 1
    origin: int = 0
    ...
    def validate_language(cls, lang: str) -> str:
        if not re.fullmatch(r"^(zh-CN|en|ko-KR|tr-TR)$", lang):
            raise ValueError("Language must be 'zh-CN', 'en', 'ko-KR', or 'tr-TR'")
        return lang
